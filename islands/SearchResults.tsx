import { KvEntry, TW_TABLE, TW_TABLE_WRAPPER, TW_TBODY, TW_TD, TW_TH, TW_THEAD, TW_TR } from "../types.ts";

interface SearchResultsProps {
  results: KvEntry[] | undefined;
        ...
        <div class={TW_TABLE_WRAPPER}>
          <table class={TW_TABLE}>
            <thead class={TW_THEAD}>
              <tr>
                <th class={TW_TH + " w-12"}>
                  <input id="selectAll" type="checkbox" name="selectAll" class="w-4 h-4"/>
                </th>
              ...
              {props.results!.map((result) => {
                return (
                  <tr class={TW_TR}>
                    ...
                    <td class={TW_TD} title={result.fullValue}>{result.value}</td>
                  </tr>
                );
              })}
            </tbody>
