...
export function AddEditConnectionDialog() {
  ...
  function cancelDialog(event: JSX.TargetedEvent<HTMLButtonElement, Event>) {
    ...
    (document.getElementById("addEditConnectionDialog")! as HTMLDialogElement).close();
  }

  return (
    <dialog id="addEditConnectionDialog" class="p-4 border-2 border-gray-700 rounded">
      <div class="mb-3">
        <p class="font-bold text-xl"><span data-type="addEdit">Add</span> new connection</p>
      </div>
      <form method="post">
        <input type="hidden" id="connectionId" name="connectionId"/>
        <div class="mt-3">
          <label for="connectionName" class="inline-block w-[70px]">Name:</label>
          <input id="connectionName" name="connectionName" class="rounded bg-blue-100 w-96 mx-2 p-2" />
        </div>
        <div class="mt-3">
          <label for="connectionLocation" class="inline-block w-[70px]">Location:</label>
